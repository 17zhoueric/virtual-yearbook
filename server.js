...
app.get('/viewProfilePage', function(req, res){
  let email = req.query.email;
  let cmd = "SELECT * FROM MyProfileTable WHERE email = ?";
  profileDB.get(cmd, email, function(err, val) {
    res.send(val);
  });
});

app.get('/getGrads', function(req, res){
  let param = req.query.search.toLowerCase().trim();
  let search_term = req.query.search2.trim();
  let gradsArray = [];
  
  console.log(param);
  console.log(search_term);

  let cmd = "SELECT * FROM MyProfileTable WHERE " + param + " = '" + search_term + "'";
  if (search_term == '') {
    cmd = "SELECT * FROM MyProfileTable";
  }
  profileDB.all(cmd, function(err, rows){
    if (err) {
      console.log(err);
    } else {
      if (rows[0]) {
        for (let i = 0; i < rows.length; i++) {
          gradsArray.push(rows[i]);
        }
        console.log(gradsArray);
        res.send(JSON.stringify(gradsArray));      
      } else {
...
passport.use(new GoogleStrategy(
  ...
  callbackURL: 'https://saber-slender-xfn.glitch.me/auth/accepted',
...
function requireUser (req, res, next) {
  ...
  if (!req.user) {
    ...
    res.redirect('/?badlogin');  //<------------------------------------------------------------------------------------------- This is the redirect to the route for an incorrect email. 
  }else {
